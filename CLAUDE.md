...
- **`tools/studio/`** — local-only publishing dashboard (binds 127.0.0.1): list posts, edit front matter, pick a cover from local files / the post's own images, upload images + deploy, delete, open in Typora. Every non-GET request must send `X-Studio: 1` (the frontend's `api()` helper does this) — that header is what stops other websites open in the browser from triggering deploys/deletes cross-origin, so new write endpoints and clients must keep it. A locally picked cover is stored as a relative path (`<slug>/cover-*.png`) and only uploaded to the image bed at publish time, alongside body images.
